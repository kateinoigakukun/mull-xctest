...
static Expected<ArrayRef<unsigned char>> getBitcodeSectionContent(const object::MachOObjectFile *object) {
    for (auto it = object->section_begin(); it != object->section_end(); ++it) {
        ...
        auto segmentName = object->getSectionFinalSegmentName(it->getRawDataRefImpl());
...
void ExtractEmbeddedFileTask::operator()(iterator begin, iterator end, Out &storage,
    ...
    for (auto it = begin; it != end; it++, counter.increment()) {
        ...
        Expected<ArrayRef<unsigned char>> contentOrErr = getBitcodeSectionContent(machOObjectFile);
        if (!contentOrErr) {
            std::stringstream errorMessage;
            errorMessage << "getBitcodeSectionContent failed: \"" << toString(contentOrErr.takeError()) << "\".";
            diagnostics.warning(errorMessage.str());
            continue;
        }
        StringRef contentData(reinterpret_cast<const char *>(contentOrErr->data()), contentOrErr->size());
