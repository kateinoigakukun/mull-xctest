...
public:
  ...
  PthreadTaskExecutor(Diagnostics &diagnostics, std::string name,
                      llvm::Optional<unsigned> StackSizeInBytes, In &in, Out &out,
...
private:
  void executeInParallel() {
    ...
    for (unsigned i = 0; i < workers; i++) {
      ...
      auto taskFn = new AsyncThreadInfo([task = std::move(tasks[i]),
                     begin, end,
      ...
      pthread_t thread;
      if ((errnum = ::pthread_create(&thread, &attr, threadFuncAsync, taskFn)) != 0)
